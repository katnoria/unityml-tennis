...
def maddpg(env, brain_name, num_agents, agent, writer, n_episodes=300, max_t=1000, print_every=50, stop_on_solve=True):
    """Train DDPG Agent
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        logger.info('Episode {}\tAverage Score: {:.2f}, Average max_t: {:.2f}, Best max_t: {}'.format(i_episode, current_score, np.mean(maxt_deque), best_maxt))
...
def main():
    ...
    parser.add_argument("--stop_on_solve", dest="stop_on_solve", action="store_true", help="Stop as soon as the environment is solved")
    parser.add_argument("--no-stop_on_solve", dest="stop_on_solve", action="store_false", help="Continue even after the environment is solved")
    parser.set_defaults(vis=True)
    parser.set_defaults(stop_on_solve=True)
    ...
    save_to_txt(args, '{}/args.txt'.format(dirname))
    ...
    scores = maddpg(env, brain_name, num_agents, agent, writer, n_episodes=args.num_episodes, max_t=args.max_t, stop_on_solve=args.stop_on_solve)
    # save all scores
    # import pdb; pdb.set_trace()
    # save_to_txt('\n'.join([score.tolist() for score in scores]), '{}/scores_multi_full.txt'.format(dirname))
    save_to_txt('\n'.join([str(score) for score in scores]), '{}/scores_multi_full.txt'.format(dirname))
